%% Pit location and information
% by Caroline Aubry-Wake, last edited Feb 12
% this script anlayses the pit information and temperature patterns
...
load('D:\2_IRPeyto\b_data_process\Pit_process\ControlPoint_TIR_TargetLocation.mat')
...
% remove the double target 9keep only initial value, as they moved during
% the TIR survey)
...
% plot results with label for control points
...
subplot(2,2,2);
imagesc(Rcrop(:,:,400)); caxis([270 280]); colormap('jet'); hold on;colorbar
hold on
scatter(CP_IR(:, 1), CP_IR(:, 2), 'ok')
c = cellstr(CP_utm_names);
dx = 0.1; dy = 0.1; % displacement so the text does not overlay the data points
...
text((CP_IR(:, 1)+ dx), (CP_IR(:, 2)+ dy), c);
title ('Northing')
 figname = 'MoraineSpatialFrame_NorthginEastingMap';
...
ExcavationLocation_TIR = IDX;
save('D:\2_IRPeyto\b_data_process\Pit_process\ExcavationLocation_TIR.mat', 'ExcavationLocation_TIR')

figure;
imagesc(VISreg); hold on
% Find pixel in X_IR that is closest to Pit location 1
clear X Y C cx cy y x row col 
for i = 1:45
